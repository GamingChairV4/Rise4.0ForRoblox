...
local function riseNotif(title, text, delay)
	local suc, res = pcall(function()
		local frame = GuiLibrary.CreateNotification(title, text, delay, "assets/InfoNotification.png")
		frame.Frame.Frame.ImageColor3 = Color3.fromRGB(119, 0, 178)
		return frame
	end)
	return (suc and res)
end

riseNotif('Rise 4', 'infrange aura (everything in universal)', 5)
...
run(function()
	...
	function whitelist:check(first)
		local whitelistloaded, err = pcall(function()
			local _, subbed = pcall(function() return game:HttpGet('https://github.com/Rise4.0ForRoblox/whitelists') end)
			local commit = subbed:find('spoofed_commit_check')
			commit = commit and subbed:sub(commit + 21, commit + 60) or nil
			commit = commit and #commit == 40 and commit or 'main'
			whitelist.textdata = game:HttpGet('https://raw.githubusercontent.com/Rise4.0ForRoblox/whitelists/'..commit..'/PlayerWhitelist.json', true)
		end)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			...
			if whitelist.data.BlacklistedUsers[tostring(lplr.UserId)] then
				task.spawn(print("hello"), lplr, whitelist.data.BlacklistedUsers[tostring(lplr.UserId)])
				return true
			end
		end
	end
			...
			if callback then
				...
				if entityLibrary.isAlive then
					...
					if selectedPosition then
						...
						ClickTP.ToggleButton(false)
						riseNotif("ClickTP", "No position found.", 1)
					end
				else
					if ClickTP.Enabled then ClickTP.ToggleButton(false) end
				end
				...
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
					...
					task.spawn(function()
						...
						if lplr.PlayerGui:FindFirstChild("Chat") and lplr.PlayerGui.Chat:FindFirstChild("Frame") and lplr.PlayerGui.Chat.Frame:FindFirstChild("ChatChannelParentFrame") and replicatedStorage:FindFirstChild("DefaultChatSystemChatEvents") then
							...
							riseNotif("ChatSpammer", "Default chat not found.", 3)
			...
			if callback then
				if textChatService.ChatVersion == Enum.ChatVersion.TextChatService then
					...
					if replicatedStorage:FindFirstChild("DefaultChatSystemChatEvents") then
						...
						riseNotif("AutoReport", "Default chat not found.", 5)
            ...
            if callback then
                game:GetService("RunService").RenderStepped:Connect(function()
                    ...
                    for ii,vv in pairs(game.workspace.Map.Beds:GetChildren()) do
                        ...
                        if (game.Players.LocalPlayer.Character:WaitForChild('HumanoidRootPart').Position - vv:WaitForChild('Hitbox').Position).Magnitude < 90 then
                            blak = vv
                        else continue
                        end
                
                        if blak then
                            game:GetService("ReplicatedStorage").Remotes.DamageBlock:InvokeServer(blak, game.Players.LocalPlayer.HotbarFolder["4"]:GetAttribute('ItemName'))
                        end
                    end
                end)
            end
...
run(function()
    local localplayer = game.Workspace:WaitForChild("PlayersContainer"):WaitForChild(game.Players.LocalPlayer.Name)
    local vapelogo = 'rbxassetid://13350860863'
    local AnonymousLocalUser = {Enabled = true}

    AnonymousLocalUser = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
        Name = 'Streamer Mode',
		HoverText = 'Protects your username.',
        Function = function(callback)
            if callback then
                for i, v in pairs(localplayer:GetDescendants()) do
                    if v.Name == 'RankIcon' then
                        v.Image = vapelogo
                    elseif v.Name == 'TextLabel' then
                        v.Text = 'Unranked (Rise Owner)'
                    elseif v.Name == 'NameLabel' then
                        v.Text = 'Me (Rise Owner)'
                    end
                end
                AnonymousLocalUser.ToggleButton(true)
            end
        end
    })
end)
...
run(function()
    local localplayer = game.Workspace:WaitForChild("PlayersContainer"):WaitForChild(game.Players.LocalPlayer.Name)
    local vapelogo = 'rbxassetid://13350860863'
    local PlayerExploit = {Enabled = true}

    PlayerExploit = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
        Name = '5 Big NNs Exploit',
		HoverText = 'NN army',
        Function = function(callback)
            if callback then
                game:GetService("RunService").RenderStepped:Connect(function()
					...
					game:GetService("ReplicatedStorage").Remotes.PurchaseItem:InvokeServer(unpack(args))
					local args = {
						[1] = Vector3.new(math.random(-300, 300), math.random(-5, 5), math.random(-300, 300)),
						[2] = "Wool"
					}
					
					game:GetService("ReplicatedStorage").Remotes.ItemRemotes.PlaceBlock:InvokeServer(unpack(args))
					end)
					PlayerExploit.ToggleButton(true)
            end
        end
    })
end)
...
run(function()
    local AutoBuy = {
        ...
        Items = {
            ...
            { Name = "Leather Armor" },
            { Name = "Iron Armor" },
            -- Add more items here as key-value pairs, where 'Name' is the item name
        }
    }

    AutoBuy = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
        Name = 'AutoBuy',
        HoverText = 'Automatically buys items',
        Function = function(callback)
            if callback then
                for _, item in ipairs(AutoBuy.Items) do
