...
local targetinfo = shared.RiseTargetInfo
...
local function GetURL(scripturl)
	if shared.RiseDeveloper then
		assert(betterisfile("Rise/"..scripturl), "File not found : Rise/"..scripturl)
		return readfile("Rise/"..scripturl)
	else
		local res = game:HttpGet("https://raw.githubusercontent.com/RiseVoidware/Risevoidware/main/"..scripturl, true)
		assert(res ~= "404: Not Found", "File not found : Rise/"..scripturl)
		return res
	end
end
...
local entity = shared.Riseentity
...
local WhitelistFunctions = shared.Risewhitelist
...
local function getcustomassetfunc(path)
	if not betterisfile(path) then
		...
		local req = requestfunc({
			Url = "https://raw.githubusercontent.com/RiseVoidware/Risevoidware/main/"..path:gsub("Rise/assets", "assets"),
			Method = "GET"
		})
...
repeat
	...
	if tabcount >= 4 or shared.RiseExecuted == nil then break end
	task.wait(1)
until tabcount >= 4 or shared.RiseExecuted == nil
...
if not shared.Risehooked then
	shared.Risehooked = true
	local tab = {31, 14, 1}
